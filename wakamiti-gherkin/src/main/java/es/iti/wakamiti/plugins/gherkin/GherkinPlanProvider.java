...
import static es.iti.wakamiti.api.repository.PlanNodeCriteria.*;
import es.iti.wakamiti.api.repository.PlanRepository;
import static es.iti.wakamiti.plugins.gherkin.GherkinConfig.STEP_MAP;
...
import es.iti.wakamiti.plugins.gherkin.parser.GherkinParser;
...
public class GherkinPlanProvider implements PlanProvider {
	...
	public void init() {
		...
		this.idTagPattern = config.get(GherkinConfig.ID_TAG_PATTERN).orElseThrow();
		this.definitionTag = config.get(GherkinConfig.DEFINITION_TAG).orElseThrow();
		this.implementationTag = config.get(GherkinConfig.IMPLEMENTATION_TAG).orElseThrow();
	}
	...
	public Optional<PlanNodeID> providePlan(List<Resource> resources) {
		if (resources.size() == 1) {
			return provideSingleFeature(resources.get(0));
		} else {
